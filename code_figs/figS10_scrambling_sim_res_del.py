...
# Deleterious counterpart of fig2, plotted on the deleterious side (ben=False). Like
# fig2, every DFE is the raw (absolute) fitness effect ΔF -- NOT a selection coefficient;
# no effect is divided by the fitness at that point in the walk. Per model:
#   * FGM   : the stored effects ΔF are used directly (traj[k] aligns with dfes[k]).
#   * p-spin: ΔF recomputed from the stored landscape J via cmn_pspin.compute_dfe(sigma, J).
#   * NK    : the stored DFEs are absolute fitness differences, but NK fitness is
#             INTENSIVE (the mean of f_i over loci) unlike the p-spin's extensive sum,
#             so they are multiplied by N to put both models on the same scale.
...
# Raw fitness effects ΔF as stored along the trajectory (traj[k] aligns with dfes[k]).
fgm_dfe1 = np.asarray(dfes[ind1])
fgm_dfe2 = np.asarray(dfes[ind2])
...
nk_n = len(nk_dfes[ind1])
nk_dfe1 = np.asarray(nk_dfes[ind1]) * nk_n
nk_dfe2 = np.asarray(nk_dfes[ind2]) * nk_n
...
create_overlapping_dfes_sim(axE, axF, sk_dfe1, sk_dfe2, xlim=15, ben=False)
# NK Plots
create_segben_sim(axG, nk_dfe1, nk_dfe2, labels=(rf'$t_1$', rf'$t_2$'), ben=False)
create_overlapping_dfes_sim(axH, axI, nk_dfe1, nk_dfe2, xlim=40, ben=False)
