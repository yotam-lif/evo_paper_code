...
# ----------------------------------------------------------------
# Plotting Helper for BDFE Distributions
# ----------------------------------------------------------------
def plot_bdfe_distribution(ax, samples, color, t_percent, model_type='fgm', param_val=None):
    ...
    # 2. Plot Untouched Histogram (Step style) ONLY
    label = rf'$t={t_percent:.0f}\%$'
...
def load_fgm_data():
    ...
    fgm_data = {}
    for _n in FGM_NS:
        file_path_fgm = f'../data/FGM/fgm_rps1000_n{_n}_sig0.05.pkl'
...
def plot_bdfe_exp_fgm(ax, reps, percents=None):
    if percents is None:
        percents = [75, 80, 85, 90]
    colors = CMR_COLORS
...
def sk_panel_fin(ax, data, N=1500, beta_arr=None, rho=1.0):
    if beta_arr is None:
...
def plot_bdfe_exp_nk(ax, nk_data, percents=None):
    if percents is None:
        percents = [75, 80, 85, 90]
    colors = CMR_COLORS
...
def main():
    ...
    out_path = os.path.join(out_dir, "fig3_dfe_dynamics.svg")
